...
# Step 2: Install Amazon CloudWatch Agent (apt only; optional)
echo ""
echo "[2/5] Installing Amazon CloudWatch Agent (via apt)..."
...
if apt-get update && apt-get install -y amazon-cloudwatch-agent 2>/dev/null; then
    ...
    echo "⚠ Warning: amazon-cloudwatch-agent apt package not available; skipping agent install"
    INSTALL_METHOD="skipped"
fi
...
if [[ "$INSTALL_METHOD" == "apt" ]]; then
    ...
    echo "ℹ Skipping service enable/start because CloudWatch Agent is not installed"
...
echo ""
echo "Log group: /northstar-cloud-solutions/npm"
echo "Log streams: {instance_id}-syslog, {instance_id}-auth, {instance_id}-docker"
echo ""
echo "Metrics collected (namespace: NorthstarCloudSolutions/System):"
